...
class TestClouDNSClient(TestCase):
    ...
    def test_illegal_module_call(self):
        ...
        self.assertEqual('illegal module nomod', str(ctx.exception))

    def test_illegal_operation_call(self):
        with self.assertRaises(ClouDNSClientException) as ctx:
            self.client._call('api', 'noop')
        self.assertEqual('api has no operation noop', str(ctx.exception))
    ...
    def test_no_response(self, api):
        with self.assertRaises(ClouDNSClientException) as ctx:
            ...
            self.client._call('api', 'get_nameservers')
        self.assertEqual('no response for api.get_nameservers',
    ...
    def test_no_api_response(self, api):
        ...
        self.assertEqual('Server Error', ctx.exception.message)
    ...
    def test_bad_request(self):
        ...
        self.assertEqual('Bad Request', ctx.exception.message)
    ...
    def test_not_found(self):
        ...
        self.assertEqual('Not Found', ctx.exception.message)
    ...
    def test_no_success(self):
        with self.assertRaises(ClouDNSClientException) as ctx:
            with requests_mock() as mock:
                ...
                self.client._call('api', 'get_nameservers')
        self.assertEqual(201, ctx.exception.response.status_code)
        self.assertEqual('No Success', ctx.exception.message)
    ...
    def test_no_status(self, api):
        with self.assertRaises(ClouDNSClientException) as ctx:
            ...
            self.client._call('api', 'get_nameservers')
        self.assertEqual('API Internal Error', ctx.exception.message)
